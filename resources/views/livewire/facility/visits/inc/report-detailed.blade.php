  <div class="card-body">
              ...
              <tbody>
                  @forelse ($supply_storages->where('entry_type', 'Main Store') as $key=>$mainStorage)
                      <tr>
                          ...
                          <td title="{{ $mainStorage->comment }}">
                              {{ Str::words($mainStorage->comment, 30, '...') }}
                          </td>
                      </tr>
                  @empty
                      <tr>
              ...
              <tbody>
                  @forelse ($supply_storages->where('entry_type', 'Other Store') as $key=>$otherStorage)
                      <tr>
                          ...
                          <td title="{{ $otherStorage->comment }}">
                              {{ Str::words($otherStorage->comment, 30, '...') }}
                          </td>
                      </tr>
                  @empty
                      <tr>
      ...
      <div>
              ...
              <tbody>
                  @forelse ($supply_storages->where('entry_type', 'Card Store') as $key=>$cardStorage)
                      <tr>
                          ...
                          <td title="{{ $cardStorage->comment }}">
                              {{ Str::words($cardStorage->comment, 30, '...') }}
                          </td>
                      </tr>
                  @empty
                      <tr>
          ...
          <h5>D5: If stock cards are kept in multiple places, how is the consumption reconciled with
              the main
              store/stock card?</h5>
      ...
      <a target="_blank" href="{{ route('facility-visit_print', $active_visit->visit_code) }}"
          class=" btn btn-sm btn-info fa fa-print"></a>
  </div>
