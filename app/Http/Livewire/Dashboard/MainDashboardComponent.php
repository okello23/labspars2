...
class MainDashboardComponent extends Component
{
    ...
    public function loadDashboardData()
    {
        ...
        switch ($this->dateRange) {
            case 'today':
                $query->whereDate('date_of_visit', Carbon::today());
                break;
            case 'week':
                $query->whereBetween('date_of_visit', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()]);
                break;
            case 'month':
                $query->whereBetween('date_of_visit', [Carbon::now()->startOfMonth(), Carbon::now()->endOfMonth()]);
                break;
            case 'custom':
                if ($this->customStartDate && $this->customEndDate) {
                    $query->whereBetween('date_of_visit', [$this->customStartDate, $this->customEndDate]);
                }
                break;
        }
        ...
        // Basic Statistics
        $this->totalVisits     = $query->count();
        $this->pendingVisits   = $query->where('status', 'Pending')->count();
        $this->completedVisits = $query->where('status', 'Approved')->count();

        // Visit Status Distribution
        $this->visitsByStatus = $query->select('status', DB::raw('count(*) as count'))
            ->groupBy('status')
            ->get();

        // Region-wise Statistics
        $this->regionWiseStats = $query->select('regions.name', DB::raw('count(*) as visits'))
        ...
        $this->equipmentStats = DB::table('fv_equipment_functionalities')
            ...
            ->whereIn('visit_id', $query->pluck('id'))
        ...
        $this->facilityStats = [
            ...
            'visited' => $query->distinct('facility_id')->count('facility_id'),
